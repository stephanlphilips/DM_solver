...
# def pulse_auxilary(time, delay, indicator):
#     trapezoid_coefficients = [[0., 0.25,0.75, 1.],
#                               [0., 0.2,0.8, 1.]][indicator]
#     x = time / delay
#     print("test: ",trapezoid_coefficients[0])
#     result = np.piecewise(x, [x <= trapezoid_coefficients[0],
#                                 x >= trapezoid_coefficients[0] and
#                                 x <= trapezoid_coefficients[1],
#                                 x >= trapezoid_coefficients[1] and
#                                 x <= trapezoid_coefficients[2],
#                                 x >= trapezoid_coefficients[2] and
#                                 x <= trapezoid_coefficients[3],
#                                 x >= trapezoid_coefficients[3] 
#                                 ], 
#                             [0.,
#                              lambda x: x/trapezoid_coefficients[1],
#                              1.,
#                              lambda x: (1.-(x-trapezoid_coefficients[3])
#                                         /trapezoid_coefficients[1]),
#                              0.])
#     return result
...
class two_qubit():
    ...
    def __init__(self, 
        ...
        # print("exchange off: ",self.exchange_residual*1e-6)
        # print("exchange on: ",self.exchange_dc*1e-6)
        # print("exchange sat: ",self.exchange_sat*1e-6)
        # print("drive sat: ",self.drive_sat*1e-6)
...
############################   CPHASE calibration   ###########################
    ...
    def calibrate_cphase(self, noise_strength, set_target, 
        ...
        def get_average_gate_fidelity(self, runs = 1000, target = None, 
            ...
            # print("shape map: ",averaged_map.shape)
            ...
            # print("phase guess: ",self.delta_z*(self.total_time*1e-9))
            # print("phase: ",ideal_phase)
            target = np.matmul((sp.linalg.expm(1j*ideal_phase[0]*self.H_zeeman_Q0)
                                @ sp.linalg.expm(1j*ideal_phase[1]*self.H_zeeman_Q1)),
                               target)
            
            # print("unitary: ",np.abs(U_ideal[1:3,1:3]))
            # print("target: ",target[1:3,1:3])
            # print("U_ideal: ",U_ideal[1:3,1:3])
            
            # print("unitary phase: ",np.angle(U_ideal[0:4,0:4]))
            # print("target phase: ",np.angle(target[0:4,0:4]))
        ...
        # print("U raw: ",np.abs(U))
        
        U = (np.conjugate(self.eig_states)
             @ U
             @ np.transpose(self.eig_states))
        
        # print("U eig: ",np.abs(U))
        ...
        for it in range(len(angle)):
            ...
            if set_target == 1:
                ...
                print("Error: target qubit is not accesable in Xiaos device")
                return None
...
##############################   CPHASE   #####################################
    ...
    def get_fidelity_cphase(self, noise_strength, b_diff = None, 
        ...
        # def envelope_fun(time):
        #         pulse_length=1.
        #         ramp_value = (1. - np.cos(2.*np.pi*time/pulse_length))/2.
        #         ramp_value = (
        #             (self.exchange_dc-self.exchange_residual)
        #             *ramp_value
        #             +self.exchange_residual
        ...
        print("exchange: ",self.exchange_dc*1e-6)
        # print("gradient: ",self.delta_z*1e-6)
        ...
        def get_average_gate_fidelity(self, runs = 1000, target = None, 
            ...
            # print("U_list: ",np.around(U_list[0],3) )
            # Calculate the averaged super operator in the Lioville 
            # superoperator form using column convention
            basis = [qt.basis(4,it) for it in range(4)]
            superoperator_basis = [basis_it1*basis_it2.dag() 
            ...
            for u in U_list:
                ...
                # print("unitary Fid: ",np.abs(temp_U)**2.)
                # print("temp_U: ",np.around(1j*temp_U,3))
                output_density = list()
                for it in range(len(superoperator_basis)):
                    temp_vec=np.array(
                        qt.operator_to_vector(temp_U*superoperator_basis[it]
            ...
            if target is None:
                ...
                #      -qt.tensor(qt.qeye(2),qt.qeye(2)))/4.
                #     )[:,:])
                # target = np.diag(np.array([1.,1.,1.,-1.],dtype = np.complex))
            ...
            # print("unitary: ",np.abs(U_ideal[1:3,1:3]))
            # print("target: ",np.around(target,3))
            # print("U_ideal: ",U_ideal[1:3,1:3])
        ...
        # print("Makhlin :",get_maklin_invariants(self))
...
#############################   resonant drive   ##############################
    ...
    def get_fidelity_crot(self, noise_strength, set_target = 0, set_phase = 0, 
        ...
        def get_average_gate_fidelity(self, runs = 1000, target_freq = set_target, 
            ...
            # print("phase guess: ",self.delta_z*(self.total_time*1e-9))
            # print("phase: ",ideal_phase)
            # target = np.matmul(sp.linalg.expm(1j*ideal_phase*self.H_zeeman_Q0),
            #                    target)
            target = np.matmul(sp.linalg.expm(1j*ideal_phase*corr_matrix),
                               target)
            # print("unitary (phase): ",np.angle(U_ideal)/np.pi)
            # print("unitary (abs): ",np.abs(U_ideal))
            
            # print("target (phase): ",np.angle(target)/np.pi)
            # print("target: ",target)
            
            # print("unitary: ",np.abs(U_ideal))
            # print("target: ",np.abs(target))
...
#############################   resonant drive   ##############################
    ...
    def get_fidelity_rot(self, noise_strength, set_qubit = 0, set_phase = 0, 
        ...
        def get_average_gate_fidelity(self, runs = 1000, 
            ...
            # print("dtype", self.phase_library["Gx"+"pi2:"+str(set_qubit)])
            # print("phase tracker origin: ",self.phase_correction_rot)
            
            # print("ideal rot: ",np.around(U_ideal,3))
            
            target = np.matmul((sp.linalg.expm(1j*ideal_phase[0]*self.H_zeeman_Q0)
                                @ sp.linalg.expm(1j*ideal_phase[1]*self.H_zeeman_Q1)),
                               target)
            
            # print("target rot: ",np.around(target,3))
        ...
        self.pulse_exchange=pgen.pulse()
        self.pulse_exchange.add_filter(self.lowpass_filter,False)
        add_ac_pulse(self,0.,self.rabi_pulse_duration, set_qubit, f_is_RWA = True)
        self.pulse_exchange.add_offset(0.)
        self.pulse_rabi.add_filter(self.lowpass_filter,False)
        self.solver_obj.add_H1_exp(self.H_heisenberg,self.pulse_exchange)
        self.solver_obj.add_H1_RWA(self.H_zeeman_ac,self.pulse_rabi)
...
#########################   compute gate sequence   ###########################
    ...
    def compute_gate_sequence(self, noise_strength, runs = 100):
        ...
        # print("Urot : ",np.around(self.U_rot,3) )
        # print("Urot sequence: ",np.around(U_rot_sequence,3) )
        ...
        # print("U_list sequence: ",np.around(U_list[0],3) )
        # Calculate the averaged super operator in the Lioville 
        # superoperator form using column convention
        basis = [qt.basis(4,it) for it in range(4)]
        superoperator_basis = [basis_it1*basis_it2.dag() 
        ...
        for u in U_list:
            ...
            temp_U = (sp.linalg.expm(-1j*self.phase_tracker_sequence[0]
                      ...
                      @ sp.linalg.expm(-1j*self.phase_tracker_sequence[1]
                                       *self.H_zeeman_Q1)
                      @ temp_U)
            # print("unitary Fid: ",np.around(temp_U,3))
    ...
    def plot_computed_mappings(self):
        self.dim = 4
        ...
        # qt.qpt_plot_combined(chi, [op_label],title = "chi")
        # plt.show()
        
        qt.qpt_plot_combined(ptm, [op_label],title = "ptm")
        plt.show()
        
        # qt.qpt_plot_combined(choi, [op_label],title = "choi")
        # plt.show()
...
if __name__ == '__main__':
    ...
    # list_noise = []
    # list_prob = []
    # for noise in np.logspace(-4,-2,51):
